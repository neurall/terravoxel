...
clock    = new THREE.Clock(); var instrow = 32, instances = instrow*instrow;; container=document.createElement('div'); document.body.appendChild(container);
stats    = new Stats(); container.appendChild( stats.dom ); scene  = new THREE.Scene(); 
renderer = new THREE.WebGLRenderer({ antialias: true }); renderer.setClearColor( 0 ); renderer.setPixelRatio( window.devicePixelRatio );renderer.setSize( window.innerWidth, window.innerHeight );container.appendChild( renderer.domElement );
camera   = new THREE.PerspectiveCamera( 90, (window.innerWidth / window.innerHeight), 0.1, 100000 );camera.position.set(0,0,instrow);camera.updateMatrix(); 
controls = new THREE.FlyControls(camera,container); controls.movementSpeed = instrow/2; controls.rotateSpeed = 1.0; controls.zoomSpeed = 1.2; controls.panSpeed = 0.8; controls.rollSpeed = Math.PI / 12; controls.autoForward = false; controls.dragToLook = true;
...
dx=Math.ceil(img.width/instrow); dy=Math.ceil(img.height/instrow);
can=document.createElement('canvas'); ctx=can.getContext('2d'); can.width=dx*instrow; can.height=dy*instrow; ctx.drawImage(img,0,0,img.width,img.height);//can.width=can.height=4;ctx.fillStyle = "#ffffff"; ctx.fillRect(0,0,4,4);ctx.fillStyle = "#808080"; ctx.fillRect(1,1,2,2);
...
for(var y=0;y<dy;y++) for(var x=0;x<dx;x++) {
	var u=(x*instrow)*(1./(dx*instrow)),ud=(1./(dx*instrow));
	var v=(y*instrow)*(1./(dy*instrow)),vd=(1./(dy*instrow));
vs='precision highp float;uniform mat4 modelViewMatrix; uniform mat4 projectionMatrix;uniform sampler2D map;uniform vec4 pos;uniform float wid;attribute vec3 position;attribute float id;varying float vid;void main() { vid=id;\
	float y=floor(vid/wid), x=mod(vid,wid), u=pos.x+(x*pos.z), v=pos.y+(y*pos.w);vec4 h=texture2D(map,vec2(u,v));\
	gl_Position = projectionMatrix * modelViewMatrix * vec4( vec3(x,y,((h.x+h.y+h.z))*10.)+position, 1.0 );}';
ps='precision highp float;uniform sampler2D map;uniform vec4 pos;uniform float wid;varying float vid;void main() {\
	float y=floor(vid/wid), x=mod(vid,wid), u=pos.x+(x*pos.z), v=pos.y+(y*pos.w);vec4 h=texture2D(map,vec2(u,v));\
	gl_FragColor = h;}';
	material = new THREE.RawShaderMaterial( {uniforms: {map: { type: "t", value: texture } ,pos : { type: "v4", value: new THREE.Vector4( u, v,ud,vd ) },wid : { type: "f",  value: instrow },},vertexShader: vs,fragmentShader: ps,transparent: false,} );
	mesh     = new THREE.Mesh( geometry, material );
	mesh.position.set((x*(instrow))-((instrow*dx)/2), (y*(instrow))-((instrow*dy)/2), 0);mesh.updateMatrix();
