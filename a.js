...
camera   = new THREE.PerspectiveCamera( 20, (window.innerWidth / window.innerHeight), 0.1, 1000 );camera.position.set(0,0,500);camera.updateMatrix(); 
...
vs='precision highp float;uniform mat4 modelViewMatrix; uniform mat4 projectionMatrix;uniform sampler2D map;uniform vec4 pos;uniform float wid;attribute vec3 position;attribute float id;varying float vid;varying vec3 poi;void main() { vid=id;\
	float y=floor(vid/wid), x=mod(vid,wid), u=pos.x+(x*pos.z), v=pos.y+(y*pos.w);vec4 h=texture2D(map,vec2(u,v));poi=position;\
	gl_Position = projectionMatrix * modelViewMatrix * vec4( vec3(x/2.,y/2.,((h.x+h.y+h.z))*3.)+position, 1.0 );}';
ps='precision highp float;uniform sampler2D map;uniform vec4 pos;uniform float wid;varying float vid;varying vec3 poi;void main() {\
	...
	gl_FragColor = h;}';
	material = new THREE.RawShaderMaterial( {side:THREE.DoubleSide, uniforms: {map: { type: "t", value: texture } ,pos : { type: "v4", value: new THREE.Vector4( u, v,ud,vd ) },wid : { type: "f",  value: instrow },},vertexShader: vs,fragmentShader: ps,transparent: true,} );
	mesh     = new THREE.Mesh( geometry, material );mesh.position.set((x*(instrow/2))-(((instrow/2)*dx)/2), (y*(instrow/2))-(((instrow/2)*dy)/2), 0);mesh.updateMatrix();scene.add( mesh ); 
}
